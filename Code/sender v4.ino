...
void setup() {
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3c, false, false)) { // Address 0x3C for 128x32
    ...
    digitalWrite(LED_BUILTIN, HIGH); // error code 1: see github
...
void Sensors() {
  ...
  delay(500);
  char nonFloTemp[4];
  char nonFloBatteryTemp[4];
  char speedBuffer[15];
  char mTemp[25];
  char bTemp[25];
  char packetToSend[55];
  ...
  if(mTemperature = 85) {
    digitalWrite(LED_BUILTIN, HIGH); // error code 3: see github
    digitalWrite(LED_BUILTIN, LOW);    
    digitalWrite(LED_BUILTIN, HIGH); //and yes I am doing like this as otherwise I would have to store an additional counter variable which is more memory intensive than just spaghetti code like this.
    digitalWrite(LED_BUILTIN, LOW);
  ...
  if(bTemperature = 85) {
    digitalWrite(LED_BUILTIN, HIGH); // error code 4: see github
    digitalWrite(LED_BUILTIN, LOW);    
    digitalWrite(LED_BUILTIN, HIGH); 
    digitalWrite(LED_BUILTIN, LOW);
    digitalWrite(LED_BUILTIN, HIGH); 
  ...
  sprintf(bTemp, "BATTERY TEMP: %sC", nonFloBatteryTemp);
  ...
  display.print(bTemp);
  // Combining the things to send as one packet
  snprintf(packetToSend, 55, "%s,%s,%s", speedBuffer, mTemp, bTemp);
